...
listę zgłoszeń, kreator i edycję zgłoszenia, kartę szczegółów zgłoszenia, obserwowanie i powiadomienia oraz centrum powiadomień użytkownika.
...
- Lista issues, karta szczegółów issue, kreator, edycja oraz centrum powiadomień są dostępne wyłącznie dla zalogowanego użytkownika.
...
- Mechanizm odświeżania listy issues i karty issue w czasie rzeczywistym jest niezależny od mechanizmu obserwowania issue. Obserwowanie służy wyłącznie do dostarczania powiadomień użytkownikowi.
...
- **Przypisany do**: jeżeli wskazany, musi pochodzić ze słownika użytkowników.
- **Acceptance criterion**: jeżeli pozycja została dodana do listy, jej treść jest wymagana.
...
- Po utworzeniu issue autor może zostać automatycznie dodany do obserwujących, jeżeli zaznaczono opcję **Obserwuj po utworzeniu**.
...
Dostęp do karty issue wymaga zalogowania.
...
- Jeżeli issue nie ma zdefiniowanych kryteriów akceptacji, widok pokazuje czytelną informację o ich braku.
...
- Dla zmian pól historia powinna pokazywać wartości **przed** i **po**, jeżeli jest to dostępne i czytelne dla użytkownika.
...
- Jeżeli użytkownik jest aktualnie zalogowany i aktywny w aplikacji, powiadomienie powinno pojawić się w centrum powiadomień oraz opcjonalnie jako sygnał natychmiastowy w UI.
- Frontend utrzymuje aktywne połączenie realtime z hubem powiadomień dla zalogowanego użytkownika.
- Frontend nasłuchuje co najmniej dwóch klas zdarzeń realtime:
  - zdarzeń dotyczących nowych powiadomień dla obserwujących użytkowników,
  - zdarzeń dotyczących zmian issue używanych do odświeżania widoków listy i szczegółów.
- Po odebraniu zdarzenia powiadomienia frontend aktualizuje bez przeładowania strony co najmniej: licznik nieprzeczytanych powiadomień oraz listę powiadomień.
- Jeżeli otwarta jest lista issues, frontend odświeża bez ręcznego refreshu co najmniej status, priorytet, liczbę obserwujących i datę ostatniej aktywności dla issue objętych zdarzeniem zmiany issue, niezależnie od tego, czy bieżący użytkownik obserwuje dane zgłoszenie.
- Jeżeli otwarta jest karta issue, frontend odświeża bez ręcznego refreshu komentarze, historię zmian, stan obserwowania, ostatnią aktywność oraz inne pola zmienione przez zdarzenie zmiany issue, niezależnie od tego, czy bieżący użytkownik obserwuje dane zgłoszenie.
...
- Użytkownik może opcjonalnie oznaczyć wiele powiadomień lub wszystkie widoczne jako przeczytane, jeżeli moduł przewiduje taką akcję zbiorczą.
