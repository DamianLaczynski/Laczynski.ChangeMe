...
To update the database use this command from the Web project folder (replace `Laczynski.ChangeMe.Backend` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../Laczynski.ChangeMe.Backend.Infrastructure/Laczynski.ChangeMe.Backend.Infrastructure.csproj -s Laczynski.ChangeMe.Backend.Web.csproj
```
